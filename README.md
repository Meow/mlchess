...
Pure self-play has a known weakness: a net that only ever plays its current self can forget how to beat its earlier selves and drift in circles. So each game is played against something drawn from a mix — `self` (both sides the current net), `snapshot` (an earlier net from `rl_snapshots/`, uniform over the newest eight), `classical` (a small classical engine built into the Rust extension: `evaluation.py`'s piece-square evaluation under a 2-ply alpha-beta, `--classical-depth`; no NNUE, no real lookahead, a materialist that plays nothing like a net), `random`, or `greedy` (the same evaluation 1 ply deep, in Python) — with a random colour. Only the current net's own moves become training rows; every game's result trains the value head. The mix slides from `--opponents` (default `self=0.6,snapshot=0.2,classical=0.1,random=0.1`) to `--opponents-final` (default `self=0.7,snapshot=0.2,classical=0.1`) over `--opponents-steps` training steps. The progress line then carries win rates per opponent (`vs random 96% snapshot 58%`), which is a strength meter you get for free; wandb has them as `vs/*`. `classical` is searched in Rust for all its games at once and costs next to nothing; `greedy` runs in Python at a millisecond or two per move, and at 16k games a 10% share was measured to halve throughput, so prefer `classical`. `rl_eval.py --opponent classical:3` plays it by hand. `--fresh` moves the previous run's checkpoint, weights and snapshots to `rl_previous_<time>/` so a new net neither overwrites them nor trains against them.
