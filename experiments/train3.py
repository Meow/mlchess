...
class ChessModel(torch.nn.Module):
  def __init__(self, input_dimension=1088, feature=512):
    ...
    self.layer3_norm = nn.LayerNorm(128)
  ...
  def forward(self, board_in, piece_in):
    ...
    data = self.layer2_norm(self.gelu(self.f2(data)))
    data = self.layer3_norm(self.gelu(self.f3(data)))
    data = self.layer3_norm(self.gelu(self.f4(data)))

    return self.f5(data)

print("intitializing training...")

model = torch.load('chess3.model') if os.path.exists('chess3.model') else ChessModel()
