...
class ChessModel(torch.nn.Module):
  def __init__(self, input_dimension=1088, feature=512):
    ...
    self.f3 = nn.Linear(feature, 256)
    self.f4 = nn.Linear(256, 256)
    self.f5 = nn.Linear(256, 64)
    self.gelu = nn.GELU()
  ...
  def forward(self, board_in, piece_in):
    ...
    data = self.layer_norm(self.gelu(self.f2(data)))
    data = self.layer2_norm(self.gelu(self.f3(data)))
    data = self.layer2_norm(self.gelu(self.f4(data)))

    return self.f5(data)

print("intitializing training...")

model = torch.load('chess4.model') if os.path.exists('chess4.model') else ChessModel()
